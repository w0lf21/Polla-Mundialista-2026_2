...
app.post('/api/admin/bracket/propagate', authMiddleware, adminMiddleware, (req, res) => {
  // Propagar todos los ganadores existentes en cascada
  const allMatches = db.prepare('SELECT * FROM matches WHERE winner IS NOT NULL AND phase != ?').all('groups');
  let propagated = 0;
...
app.put('/api/admin/matches/:id', authMiddleware, adminMiddleware, (req, res) => {
  ...
  // Actualizar marcadores (siempre sobreescribir, no COALESCE para scores)
  db.prepare(`
    UPDATE matches SET
      home_score = ?,
      away_score = ?,
  ...
  if (autoWinner) {
    ...
    // Propagar ganador al siguiente partido automáticamente
