...
const WORLD_CUP_START_UTC = new Date('2026-06-11T19:00:00Z');
const LOCK_BEFORE_START_MS = 15 * 60 * 1000;

function areGroupPredictionsLocked() {
  return new Date() >= new Date(WORLD_CUP_START_UTC.getTime() - LOCK_BEFORE_START_MS);
}
...
function isDailyBetLocked(matchDate, matchTime) {
  ...
  return new Date() >= new Date(start.getTime() - 5 * 60 * 1000);
}
...
app.get('/api/predictions/lock-status', (req, res) => {
  ...
  const lockTime = new Date(WORLD_CUP_START_UTC.getTime() - LOCK_BEFORE_START_MS);
  res.json({ locked, lockTime: lockTime.toISOString(), lockTimeEcuador: formatEcuadorTime(lockTime) });
});
...
app.post('/api/predictions', authMiddleware, (req, res) => {
  if (areGroupPredictionsLocked())
    return res.status(403).json({ error: 'Las predicciones ya estan cerradas (15 min antes del primer partido).' });
  ...
  if (!db.prepare('SELECT id FROM matches WHERE id = ?').get(match_id))
    return res.status(404).json({ error: 'Partido no existe' });
...
app.post('/api/pollas/:polla/register', authMiddleware, (req, res) => {
  ...
  if (polla === 'groups' && areGroupPredictionsLocked())
    return res.status(400).json({ error: 'La inscripción a grupos ya cerró.' });
