...
// Migración: corregir labels de SF-1..4 (Cuartos) y SF-5..6 (Semifinales)
...
app.post('/api/admin/bracket/generate', authMiddleware, adminMiddleware, (req, res) => {
  const { selectedThirds } = req.body || {};
  // selectedThirds: array de 8 códigos de grupo (ej: ['A','C','D','E','F','G','H','I'])
  if (!selectedThirds || selectedThirds.length !== 8)
    return res.status(400).json({ error: 'Debes seleccionar exactamente 8 terceros.' });
  ...
  for (const g of GROUPS) {
    ...
    if (standing[2] && selectedThirds.includes(g)) thirds[g] = standing[2].code;
  }

  // Tabla de combinaciones FIFA (Annex C) — mapeo de 8 grupos → cruces de terceros
  // Formato: key = grupos ordenados, value = {R32-3, R32-5, R32-7, R32-8, R32-9, R32-10, R32-13, R32-15}
  // Basado en la tabla oficial de Wikipedia
  const key = [...selectedThirds].sort().join('');

  // Cruces fijos (independientes de terceros)
  const fixedMatches = {
  ...
  // Cruces con terceros — tabla simplificada de las 495 combinaciones
  // Usando los partidos que involucran terceros: R32-3(1E), R32-5(1I), R32-7(1A), R32-8(1L), R32-9(1G), R32-10(1D), R32-13(1B), R32-15(1K)
  // Determinamos qué tercero va contra cada primero según los grupos clasificados
  const thirdMatches = calcThirdPlaceMatches(selectedThirds, thirds, winners);
...
function calcThirdPlaceMatches(selectedGroups, thirds, winners) {
  // Según reglamento FIFA, los 8 terceros clasificados se asignan así:
  // R32-3: 1°E vs 3ro de A/B/C/D/F
  // R32-5: 1°I vs 3ro de C/D/F/G/H
  // R32-7: 1°A vs 3ro de C/E/F/H/I
  // R32-8: 1°L vs 3ro de E/H/I/J/K
  // R32-9: 1°G vs 3ro de A/E/H/I/J
  // R32-10: 1°D vs 3ro de B/E/F/I/J
  // R32-13: 1°B vs 3ro de E/F/G/I/J
  // R32-15: 1°K vs 3ro de D/E/I/J/L

  const slots = [
    { matchId: 'R32-3',  home: winners['E'], candidates: ['A','B','C','D','F'] },
    { matchId: 'R32-5',  home: winners['I'], candidates: ['C','D','F','G','H'] },
    { matchId: 'R32-7',  home: winners['A'], candidates: ['C','E','F','H','I'] },
    { matchId: 'R32-8',  home: winners['L'], candidates: ['E','H','I','J','K'] },
    { matchId: 'R32-9',  home: winners['G'], candidates: ['A','E','H','I','J'] },
    { matchId: 'R32-10', home: winners['D'], candidates: ['B','E','F','I','J'] },
    { matchId: 'R32-13', home: winners['B'], candidates: ['E','F','G','I','J'] },
    { matchId: 'R32-15', home: winners['K'], candidates: ['D','E','I','J','L'] },
  ];
  ...
  for (const slot of slots) {
    // Encontrar el mejor tercero disponible que sea candidato para este slot
    const available = selectedGroups
      .filter(g => slot.candidates.includes(g) && !assigned.has(g))
      .sort((a, b) => {
        // Priorizar por ranking (ya vienen ordenados, aquí usamos el índice)
        return selectedGroups.indexOf(a) - selectedGroups.indexOf(b);
      });

    if (available.length > 0) {
      const chosen = available[0];
      assigned.add(chosen);
      result[slot.matchId] = { home: slot.home, away: thirds[chosen] };
    }
  }
...
app.post('/api/admin/bracket/propagate', authMiddleware, adminMiddleware, (req, res) => {
  // Mapa de qué partido alimenta a qué partido y en qué posición
  ...
  const update = db.prepare('UPDATE matches SET home_team = ? WHERE id = ?');
  const updateAway = db.prepare('UPDATE matches SET away_team = ? WHERE id = ?');

  let propagated = 0;
  const propagateAll = db.transaction(() => {
    for (const rule of propagationMap) {
      const fromMatch = db.prepare('SELECT * FROM matches WHERE id = ?').get(rule.from);
      if (!fromMatch || !fromMatch.winner) continue;

      let team;
      if (rule.useLoser) {
        // Para 3er puesto usamos el perdedor
        team = fromMatch.winner === fromMatch.home_team ? fromMatch.away_team : fromMatch.home_team;
      } else {
        team = fromMatch.winner;
      }

      if (!team) continue;
      if (rule.pos === 'home') update.run(team, rule.to);
      else updateAway.run(team, rule.to);
      propagated++;
    }
  });
  propagateAll();
...
app.put('/api/admin/matches/:id', authMiddleware, adminMiddleware, (req, res) => {
  ...
  // Auto-calcular winner si hay marcador y no se pasó explícitamente
  const match = db.prepare('SELECT * FROM matches WHERE id = ?').get(matchId);
