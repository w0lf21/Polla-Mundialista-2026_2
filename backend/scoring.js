const POINTS = {
  ...
  // Eliminatorias - 90 min
  KO_EXACT_90: 5,
  KO_WINNER_DIFF: 3,
  KO_WINNER: 2,

  // Eliminatorias - con penales
  KO_EXACT_90_EXACT_PEN: 8,
  KO_DRAW_EXACT_PEN: 5,
  KO_DRAW_WINNER_PEN: 3,
...
function calcGroupMatchPoints(pred, real) {
  ...
  if (realResult === 'D') return POINTS.CORRECT_DRAW;
...
function calcKOMatchPoints(pred, real) {
  ...
  const rh = parseInt(real.home_score);
  const ra = parseInt(real.away_score);
  const realIs90Draw = rh === ra;
  const realWinner = real.winner;

  // Si el partido termino en 90 min sin empate (no hubo penales)
  if (!realIs90Draw) {
    if (pred.pred_home == null || pred.pred_away == null) return 0;
    ...
    if (ph === rh && pa === ra) return POINTS.KO_EXACT_90;
  ...
  // Partido fue a penales (empate en 90 min)
  // pred_home/pred_away = marcador en 90 min
  // pred_winner = ganador en penales
  // pred_pen_home/pred_pen_away = marcador en penales

  const predIs90Draw = pred.pred_home != null && pred.pred_away != null &&
    parseInt(pred.pred_home) === parseInt(pred.pred_away);
  const predExact90 = pred.pred_home != null && pred.pred_away != null &&
  ...
  // Marcador exacto 90 min + penales exactos
  if (predExact90 && exactPenales && correctWinner) {
    return POINTS.KO_EXACT_90_EXACT_PEN;
  }

  // Empate exacto en 90 min + ganador correcto en penales exactos
  if (predExact90 && exactPenales && !correctWinner) {
    return POINTS.KO_DRAW_EXACT_PEN;
  }

  // Empate en 90 min (no exacto) + penales exactos + ganador correcto
  if (predIs90Draw && exactPenales && correctWinner) {
    return POINTS.KO_DRAW_EXACT_PEN;
  }

  // Empate correcto + ganador correcto en penales (no exacto)
  if (predExact90 && correctWinner) {
    return POINTS.KO_DRAW_WINNER_PEN;
  }

  if (predIs90Draw && correctWinner) {
    return POINTS.KO_DRAW_WINNER_PEN;
  }
...
function calcUserTotalPoints(db, userId) {
  ...
  for (const m of matches) {
    ...
    if (pts === POINTS.EXACT_SCORE || pts === POINTS.KO_EXACT_90 || pts === POINTS.KO_EXACT_90_EXACT_PEN) exactScores++;
  }
