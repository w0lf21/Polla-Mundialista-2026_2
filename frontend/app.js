...
const app = {
  ...
  async renderPodium(main) {
    // === PODIO PRONOSTICADO (de los pronósticos del usuario) ===
    ...
    // === PODIO REAL (de los resultados cargados por el admin) ===
  ...
  async renderLeaderboard(main) {
    ...
    try {
      ...
      const me = lb.find(u => u.id === this.user.id);
      document.getElementById('header-points').textContent = (me ? me.points : 0) + ' pts';
      const pot = lb.filter(u => u.paid_entry).length * 10;

      main.innerHTML = `
        <h2>Ranking · Pozo $${pot}</h2>
        <div class="grid-2" style="margin-bottom:1rem;">
          <div class="metric-card"><div class="metric-label">Participantes</div><div class="metric-value">${lb.length}</div></div>
          <div class="metric-card"><div class="metric-label">Pagados</div><div class="metric-value">${lb.filter(u=>u.paid_entry).length} / ${lb.length}</div></div>
          <div class="metric-card"><div class="metric-label">Pozo total</div><div class="metric-value">$${pot}</div></div>
          <div class="metric-card"><div class="metric-label">Reparticion</div><div class="metric-value" style="font-size:14px">50 / 30 / 20%</div></div>
        </div>
        <table class="leaderboard-table">
          <thead><tr><th>#</th><th>Participante</th><th style="text-align:center">Aciertos</th><th style="text-align:center">Exactos</th><th style="text-align:right">Puntos</th></tr></thead>
          <tbody>
            ${lb.map((u, i) => {
              ...
              return `<tr>
                <td><span class="rank-medal ${medal}">${rank}</span></td>
                <td class="user-cell"><span class="avatar">${init}</span><span>${u.display_name}${u.paid_entry?'':'<span class="chip unpaid">sin pago</span>'}</span></td>
                <td style="text-align:center">${u.correctPredictions}</td>
                <td style="text-align:center">${u.exactScores}</td>
                <td style="text-align:right;font-weight:700">${u.points}</td>
              </tr>`;
            }).join('')}
          </tbody>
  ...
  renderRules(main) {
    main.innerHTML = `
      <h2>Reglas y puntuacion</h2>
      <div class="card"><h3>Inscripcion y pozo</h3><p style="font-size:14px;color:var(--color-text-muted);line-height:1.8">Cada participante paga $10. Al final el pozo se reparte: 50% al primero, 30% al segundo, 20% al tercero.</p></div>
      <div class="card"><h3>Cierre de predicciones</h3><p style="font-size:14px;color:var(--color-text-muted);line-height:1.8">Las predicciones se cierran <strong>15 minutos antes del primer partido</strong>. Las apuestas diarias se cierran <strong>5 minutos antes</strong> de cada partido. Todos los horarios son en hora Ecuador (GMT-5).</p></div>
      <div class="card"><h3>Formato FIFA 2026</h3><p style="font-size:14px;color:var(--color-text-muted);line-height:1.8">48 equipos en 12 grupos. Clasifican los 2 primeros de cada grupo + 8 mejores terceros = 32 equipos. Fases: Dieciseisavos → Octavos → Cuartos → Semis → Final.</p></div>
      <div class="card"><h3>Eliminatorias dinamicas</h3><p style="font-size:14px;color:var(--color-text-muted);line-height:1.8">La app calcula automaticamente que equipos juegan en cada partido segun tus marcadores pronosticados. Si predices empate en 90 min, ingresa el marcador de penales para definir quién avanza. El podio se determina automaticamente segun tus pronosticos de la Gran Final y el Tercer puesto.</p></div>
      <div class="card"><h3>Mini-Pollas</h3><p style="font-size:14px;color:var(--color-text-muted);line-height:1.8">Pollas independientes por fase eliminatoria. Se abren entre rondas y se cierran 5 minutos antes del primer partido de cada fase. Cualquier usuario puede participar aunque no haya estado en la polla general. Reparto: 70% al primero, 30% al segundo.</p></div>
      <div class="card">
        <h3>Puntos · Grupos</h3>
        <ul style="font-size:14px;line-height:2;list-style:none">
          <li>🎯 Marcador exacto: <strong>5 puntos</strong></li>
          <li>📏 Ganador + diferencia correcta: <strong>3 puntos</strong></li>
          <li>✅ Solo ganador correcto: <strong>2 puntos</strong></li>
          <li>🤝 Empate exacto: <strong>3 puntos</strong></li>
        </ul>
      </div>
      <div class="card">
      ...
      <div class="card">
        ...
        <ul style="font-size:14px;line-height:2;list-style:none">
          <li>🏆 Marcador exacto 90 min + penales exactos + ganador: <strong>8 puntos</strong></li>
          <li>🎯 Empate exacto + penales exactos: <strong>5 puntos</strong></li>
          <li>✅ Empate correcto + ganador correcto penales: <strong>3 puntos</strong></li>
          <li>👍 Solo ganador correcto: <strong>2 puntos</strong></li>
        </ul>
      </div>
      <div class="card">
        <h3>Puntos · Podio</h3>
        <ul style="font-size:14px;line-height:2;list-style:none">
          <li>🥇 Campeon: <strong>15 puntos</strong></li>
          <li>🥈 Subcampeon: <strong>10 puntos</strong></li>
          <li>🥉 Tercer lugar: <strong>6 puntos</strong></li>
        </ul>
      </div>
      <div class="card"><h3>Apuestas diarias</h3><p style="font-size:14px;color:var(--color-text-muted);line-height:1.8">Apuesta $2 por partido. Quienes aciertan el marcador exacto se reparten el pote. En caso de no haber un acierto exacto, se reparte el pote entre quienes acertaron el ganador. En caso de no acertar el ganador, puedes usar esos $2 para el siguiente partido. Los partidos diarios no suman puntos al ranking general.</p></div>
    `;
  },
