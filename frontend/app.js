...
const app = {
  ...
  async renderKnockout(main) {
    ...
    html += `<div class="notice">Ingresa el marcador de cada partido. Si hay empate aparecerán los campos de penales. El ganador se calcula automáticamente y pasa a la siguiente ronda. <strong>Predice de arriba hacia abajo.</strong>${!locked && this.lockStatus.lockTimePolla2Ecuador ? ` Se cierran <strong>5 minutos antes del primer partido de dieciseisavos</strong> (${this.lockStatus.lockTimePolla2Ecuador} hora Ecuador).` : ''}</div>`;
    if (locked) {
...
// ── REGLAS ──────────────────────────────────────────────────────────────────
  ...
  async renderAdminMiniPollas() {
    ...
    try {
      ...
      // Cargar leaderboards de cada fase
      const lbData = {};
  ...
  renderAdminMatches() {
    ...
    const phases = [
      ...
      { key: 'sf_qf',  label: 'Cuartos', ids: ['SF-1','SF-2','SF-3','SF-4'] },
    ...
    container.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:8px">
        <div style="font-size:13px;color:var(--color-text-muted)">Los resultados se guardan automáticamente al salir de cada campo.</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          ${groupsComplete ? `<button class="btn-primary" style="width:auto;background:linear-gradient(135deg,#1a5c8a,#0f3d6b)" onclick="app.openBracketGenerator()">🔄 Generar Eliminatorias</button>` : ''}
      ...
      ${phases.map(p => {
        ...
        if (!matches.length) return '';
        return `
          <details style="margin-bottom:10px" ${p.key === 'groups' ? 'open' : ''}>
                ...
                return `<div class="user-row" data-admin-match="${m.id}" style="grid-template-columns:1fr auto;${hasResult?'border-left:2px solid var(--color-success)':''}">
                  <div style="font-size:13px">
                    ...
                    <div style="display:flex;gap:4px;flex-wrap:wrap;align-items:center;margin-bottom:6px">
                      ...
                      <input type="number" min="0" max="20" data-field="home_score" value="${m.home_score??''}" style="width:46px;text-align:center" placeholder="—">
                      <span style="color:var(--color-text-muted)">—</span>
                      <input type="number" min="0" max="20" data-field="away_score" value="${m.away_score??''}" style="width:46px;text-align:center" placeholder="—">
                    </div>
                    <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">
                      <label style="font-size:12px;color:var(--color-text-muted)">Penales:</label>
                      ...
                      <select data-field="winner" style="flex:1">
                        <option value="">—</option>
                        ${homeTeam ? `<option value="${homeTeam.code}" ${m.winner===homeTeam.code?'selected':''}>${homeTeam.flag} ${homeTeam.name}</option>` : ''}
                        ${awayTeam ? `<option value="${awayTeam.code}" ${m.winner===awayTeam.code?'selected':''}>${awayTeam.flag} ${awayTeam.name}</option>` : ''}
                      </select>
                    </div>
                  </div>
                  <span class="match-save-status" style="font-size:14px;width:20px;text-align:center"></span>
  ...
  async openBracketGenerator() {
    const main = document.getElementById('admin-matches');
    const statusDiv = document.getElementById('admin-matches-msg');
    if (statusDiv) { statusDiv.textContent = 'Calculando clasificados...'; statusDiv.style.color = 'var(--color-text-muted)'; }

    try {
      const data = await this.api('/admin/bracket/standings');
      const { winners, runnersUp, thirds } = data;

      // Mostrar modal con los terceros para seleccionar 8
      const modalHtml = `
        <div id="bracket-modal" style="position:fixed;inset:0;background:rgba(0,0,0,0.8);z-index:1000;display:flex;align-items:center;justify-content:center;padding:16px">
          <div style="background:var(--color-surface);border:1px solid var(--color-border);border-radius:var(--radius-lg);padding:1.5rem;max-width:520px;width:100%;max-height:90vh;overflow-y:auto">
            <h3 style="margin-bottom:12px">🔄 Generar bracket eliminatorio</h3>
            <div style="font-size:13px;color:var(--color-text-muted);margin-bottom:16px">
              Los 1ros y 2dos de cada grupo se asignan automáticamente. Selecciona los <strong>8 mejores terceros</strong> que clasificaron:
            </div>

            <div style="margin-bottom:16px">
              <div style="font-size:12px;font-weight:600;color:var(--color-primary);margin-bottom:8px">Terceros rankeados (selecciona 8):</div>
              ${thirds.map((t, i) => {
                const teamInfo = this.teams.find(tm => tm.code === t.code);
                return `
                  <label style="display:flex;align-items:center;gap:8px;padding:6px 8px;border-radius:var(--radius-md);cursor:pointer;border:1px solid var(--color-border);margin-bottom:4px;${i < 8 ? 'background:rgba(201,168,76,0.05)' : ''}">
                    <input type="checkbox" class="third-check" value="${t.group}" ${i < 8 ? 'checked' : ''} style="width:16px;height:16px">
                    <span style="font-size:12px;color:var(--color-text-muted);width:20px">${i+1}.</span>
                    <span>${teamInfo?.flag||''} ${teamInfo?.name||t.code}</span>
                    <span style="font-size:11px;color:var(--color-text-muted)">Grupo ${t.group}</span>
                    <span style="margin-left:auto;font-size:11px;color:var(--color-text-muted)">${t.pts}pts · GD:${t.gd>0?'+':''}${t.gd}</span>
                  </label>`;
              }).join('')}
            </div>

            <div id="third-count-msg" style="font-size:12px;color:var(--color-primary);margin-bottom:12px">8 seleccionados ✓</div>

            <div style="display:flex;gap:8px;justify-content:flex-end">
              <button class="btn-sm btn-ghost" onclick="document.getElementById('bracket-modal').remove()">Cancelar</button>
              <button class="btn-primary" style="width:auto" onclick="app.generateBracket()">Generar bracket</button>
            </div>
          </div>
        </div>`;
      ...
      document.querySelectorAll('.third-check').forEach(cb => {
        cb.addEventListener('change', () => {
          const count = document.querySelectorAll('.third-check:checked').length;
          const msg = document.getElementById('third-count-msg');
          if (msg) {
            msg.textContent = count === 8 ? '8 seleccionados ✓' : `${count}/8 seleccionados`;
            msg.style.color = count === 8 ? 'var(--color-success)' : 'var(--color-danger)';
          }
        });
      });
  ...
  async generateBracket() {
    const checks = document.querySelectorAll('.third-check:checked');
    if (checks.length !== 8) {
      alert('Debes seleccionar exactamente 8 terceros.');
      return;
    }
    const selectedThirds = Array.from(checks).map(c => c.value);
    try {
      await this.api('/admin/bracket/generate', {
        method: 'POST',
        body: JSON.stringify({ selectedThirds })
      });
      ...
      if (msg) {
        msg.textContent = '✓ Bracket generado correctamente.';
        msg.style.color = 'var(--color-success)';
        setTimeout(() => msg.textContent = '', 3000);
      }
    } catch (e) { alert('Error: ' + e.message); }
  ...
  async saveAdminMatch(row) {
    ...
    // Solo guardar si hay al menos un marcador
    if (body.home_score == null && body.away_score == null && !body.home_team && !body.away_team) return;
    ...
    try {
      ...
      // Actualizar borde verde si tiene resultado
