...
// ── REGLAS ──────────────────────────────────────────────────────────────────
  ...
  renderAdminMatches() {
    ...
    const r16Complete = this.matches.filter(m => m.phase === 'r16').some(m => m.home_score != null);

    container.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;flex-wrap:wrap;gap:8px">
        <div style="font-size:13px;color:var(--color-text-muted)">Los resultados se guardan y propagan automáticamente.</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          ...
          ${r16Complete ? `<button class="btn-sm btn-ghost" onclick="app.repropagate()" title="Recalcular todos los ganadores en cascada">⚡ Repropagar</button>` : ''}
  ...
  async repropagate() {
    ...
    try {
      const result = await this.api('/admin/bracket/propagate', { method: 'POST' });
      if (msg) {
        msg.textContent = `⚡ Ganadores propagados correctamente.`;
        msg.style.color = 'var(--color-success)';
        setTimeout(() => msg.textContent = '', 3000);
      }
      await this.loadData();
  ...
  async saveAdminMatch(row) {
    ...
    const homeCode = row.dataset.home || null;
    const awayCode = row.dataset.away || null;
    const body = {};
    ...
    if (body.home_score > body.away_score) {
      body.winner = homeCode;
    } else if (body.away_score > body.home_score) {
      body.winner = awayCode;
    } else if (body.pen_home != null && body.pen_away != null) {
      if (body.pen_home > body.pen_away) body.winner = homeCode;
      else if (body.pen_away > body.pen_home) body.winner = awayCode;
    }
