          ...
          statusHtml = `
            <div style="margin-top:8px;padding:8px;background:var(--color-surface-2);border-radius:var(--radius-md);font-size:12px">
              ...
              ${myResult ? (myResult.won
                ...
                : `<div style="color:var(--color-text-muted)">Tu pronóstico: ${myResult.pred}</div>`) : ''}
            </div>`;
        }
...
// ── REGLAS ──────────────────────────────────────────────────────────────────
  ...
  async renderAdminTodayBets() {
    ...
    if (!container) return;
    try {
      const data = await this.api('/admin/daily-bets/today');
      if (!data.matches.length) {
        container.innerHTML = `<div class="empty-state"><div class="empty-state-icon">📅</div><p>No hay partidos hoy (${data.date} hora Ecuador).</p></div>`;
        return;
      }

      let html = `<h3>Apuestas del día · ${data.date}</h3>`;

      data.matches.forEach(m => {
        const { match, totalBets, totalPot, potType, carried, perWinner, winners, bets } = m;
        ...
        if (finished) {
          let potMsg = '', potColor = 'var(--color-text-muted)';
          if (carried) { potMsg = `⏩ Nadie acertó — pote $${totalPot.toFixed(2)} acumulado al siguiente`; }
          else if (potType === 'exacto') { potMsg = `🎯 Marcador exacto — ${winners.length} ganador${winners.length>1?'es':''} · $${perWinner.toFixed(2)} c/u`; potColor = 'var(--color-success)'; }
          else if (potType === 'ganador') { potMsg = `✅ Ganador correcto — ${winners.length} ganador${winners.length>1?'es':''} · $${perWinner.toFixed(2)} c/u`; potColor = 'var(--color-success)'; }
          statusHtml = `<div style="font-size:12px;color:${potColor};margin:6px 0">${potMsg}</div>`;
        }
        ...
        html += `
          <div style="margin-bottom:12px;padding:10px;background:var(--color-surface-2);border-radius:var(--radius-md);border:1px solid var(--color-border)">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:6px">
              ...
              <div style="font-size:12px;color:var(--color-text-muted)">${timeStr} · Pote: $${totalPot.toFixed(2)}</div>
            </div>
            ${statusHtml}
            ${totalBets > 0 ? `
            <details style="margin-top:6px">
              <summary style="cursor:pointer;font-size:12px;color:var(--color-text-muted)">${totalBets} apuesta${totalBets!==1?'s':''}</summary>
              <div style="margin-top:6px;display:grid;gap:3px">
                ${bets.map(b => `
                  <div style="display:flex;align-items:center;gap:8px;font-size:12px;padding:3px 0">
                    <span style="flex:1;${b.won?'font-weight:700;color:var(--color-success)':''}">${b.display_name}</span>
                    <span style="color:var(--color-text-muted)">${b.pred}</span>
                    <span>$${b.amount}</span>
                    ${b.won ? '<span class="chip paid">Ganó</span>' : ''}
                  </div>`).join('')}
              </div>
            </details>` : `<div style="font-size:12px;color:var(--color-text-muted)">Sin apuestas aún</div>`}
          </div>`;
      });
